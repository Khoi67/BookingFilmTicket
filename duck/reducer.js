...
const movieBookingReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case PAY: {
      console.log("asd");
      const gheDangChon = state.chairBookingList.map((e)=>e.soGhe)
      const data = state.chairList.map((item) => {
        const gheNew = item.danhSachGhe.map((chair) => {
          if(gheDangChon.includes(chair.soGhe)){
            chair.daDat = true
          }
          return chair
        });
      ...
      return {...state, chairList: data, chairBookingList: []}
    }
